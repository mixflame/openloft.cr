...
window.gotDevices = (mediaDevices) => {
  ...
  window.textEditor = new HtmlTextCollabExt.CollaborativeTextArea({
    ...
    onInsert: (index, value) => {
      ...
      let changes = Automerge.getChanges(currentDoc, newDoc);
      let base64 = btoa(String.fromCharCode.apply(null, changes[0]));
      window.text_channel.push("message_new", {operation: "insert", value: value, index: index, user_id: currentUser, room: room, name: window.name, changes: base64});
      currentDoc = newDoc;
    },
    onDelete: (index, length) => {
      ...
      let changes = Automerge.getChanges(currentDoc, newDoc);
      let base64 = btoa(String.fromCharCode.apply(null, changes[0]));
      window.text_channel.push("message_new", {operation: "delete", index: index, length: length, user_id: currentUser, room: room, name: window.name, changes: base64});
      currentDoc = newDoc;
    },
